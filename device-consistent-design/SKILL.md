...
- **Typography** — a full role-based hierarchy (display/hero text, section headings, sub-headings, card/component headings, lead paragraphs, body text, supporting/secondary text, captions/fine print, labels, eyebrows, button/CTA text, navigation text, numerals/stats, testimonial or quote text, and any other role actually present on the page). Every element sharing a role must match in size, weight, line-height, and letter-spacing at a given tier, and the whole hierarchy must scale between tiers in a way that preserves relative priority — nothing that was subordinate becomes visually dominant after scaling, and nothing dominant collapses into looking like supporting text.
- **Spacing** — section padding, container gutters, card/component internal padding, margins between headings/paragraphs/CTAs, gaps in grids and flex layouts, spacing between repeated items (cards, list rows, form fields). One coherent spacing rhythm per tier, not independently invented values per section.
