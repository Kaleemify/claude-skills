...
- **Consistency is not sameness.** The skill makes the design *system* consistent (type scale, spacing, color, component styles) — it never asks sections to look alike. Each section gets its own layout and composition for its content, repeating one layout pattern section after section is treated as a defect, and the evidence-based standards act as a quality floor, not a style guide that pulls a distinctive direction toward a conventional one.
...
- **Drift-recognition heuristics** for existing sites: inline styles overriding a shared rule, the same literal value duplicated across unrelated components, one sibling in a repeating pattern sized differently with no stated reason, property overrides that interact badly with inherited rules, and the same layout pattern repeated section after section (template drift).
...
- **Verification is code-level first for technical correctness.** Static analysis, lint, type-checking, and build tools are the default for confirming the implementation is correct; opening a browser to re-test everything is reserved for what genuinely can't be confirmed from the code itself — which keeps token and time cost down without sacrificing accuracy. The one standing exception is visual design quality: any work that creates or changes how a page looks is always rendered and reviewed in screenshots at each device tier, because only seeing it shows whether it is distinctive and premium or generic and repetitive.
